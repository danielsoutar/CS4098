...
class myTest(unittest.TestCase):
    ...
    def test_Naive_get_key(self):
        cell = np.array([0, 3, 0, 3])
        key = Naive.get_key(cell)
        self.assertTrue(key == (0, 3, 0, 3))

    def test_Naive_invalid_key(self):
        cell = None
        with self.assertRaises(TypeError):
            Naive.get_key(cell)

    def test_Naive_get_Cluster_no_cluster(self):
        cell = np.array([0, 3, 0, 3])
        image_clusters = {}
        # Should fail when empty
        with self.assertRaises(KeyError):
            Naive.get_cluster(image_clusters, cell)
        image_clusters = {(0, 3, 0, 2): 1}
        # Should fail when no key there that matches.
        with self.assertRaises(KeyError):
            Naive.get_cluster(image_clusters, cell)

    def test_Naive_get_Cluster_cluster_present(self):
        cell = np.array([0, 3, 0, 3])
        a = Naive.Cluster(cell)
        image_clusters = {(0, 3, 0, 3): a}
        b = Naive.get_cluster(image_clusters, cell)
        self.assertTrue(a is b)

    def test_Naive_pure_get_Cluster_no_cluster(self):
        cell = np.array([0, 3, 0, 3])
        a = Naive.Cluster(cell)
        b = Naive.Cluster(np.array([0, 2, 0, 2]))
        # Pure variant assumes that image_clusters is a LIST.
        image_clusters = []
        with self.assertRaises(IndexError):
            Naive.pure_get_cluster(image_clusters)
        image_clusters.append(b)
        c = Naive.pure_get_cluster(image_clusters)
        self.assertFalse(a is c)

    def test_Naive_pure_get_Cluster_cluster_present(self):
        cell = np.array([0, 3, 0, 3])
        image_clusters = []
        a = Naive.Cluster(cell)
        image_clusters.append(a)
        b = Naive.pure_get_cluster(image_clusters)
        self.assertTrue(a is b)

    def test_Naive_create_cluster_invalid_cell(self):
        cell = None
        image_clusters = {}
        with self.assertRaises(TypeError):
            Naive.create_cluster(cell, image_clusters)
        cell = (0, 0)
        with self.assertRaises(IndexError):
            Naive.create_cluster(cell, image_clusters)

    def test_Naive_create_cluster_valid_cell_and_cluster(self):
        cell = np.array([0, 3, 0, 3])
        image_clusters = {}
        Naive.create_cluster(cell, image_clusters)
        self.assertTrue(len(Naive.get_cluster(image_clusters, cell).cells) == 1)
        c = Naive.get_cluster(image_clusters, cell).cells[0]
        self.assertTrue(cell is c)

    def test_Naive_pure_create_cluster_invalid_cell(self):
        pass
